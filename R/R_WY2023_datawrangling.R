...
smelt_data <-read.csv(file.path(data_root,"WY2023_deltasmelt_catch.csv")) %>% filter(Marked=="marked") %>%  
  ...
  mutate(Survey = str_replace_all(Survey, "SWP Salvage", "SWP")) 
...
edsm_data <- read_excel(file.path(data_root,"22_23Phase1_Brian.xlsx")) %>% mutate(Count=ifelse(is.na(Count),0,Count)) %>%
  ...
  filter(GearConditionCode != 4) %>% group_by(StationCode,SampleDate) %>%
...
skt_data <- read_excel(file.path(data_root,"SKT 2022-2023 Data.xlsx")) %>% rename(SKT_Catch = 'DS Catch') %>%
  ...
  filter(Year==2023) %>%
  select(SampleDate,SKT_Catch,StationCode,SurveyNumber) %>% mutate(Survey="SKT",StationCode=as.character(StationCode)) 
...
salvage_data <- read.csv(file.path(data_root,"SalvageExport.csv")) %>% rename(SampleDate=Sample.Date,Survey=Facility) %>%
  mutate(SampleDate=as.Date(SampleDate, "%Y-%m-%d")) %>% select(-Species) %>%
...
temp_date_data <- release_info %>% select(Tag,FirstDayRelease,Total,Release.Method) %>%
...
temp_catch_data <- smelt_data_join %>% group_by(SampleDate,Survey,StationCode,Tag) %>% summarise(Count=sum(Count)) %>%
  # Convert "Count" column to just 1s and 0s
  mutate(Count=ifelse(Count>1,1,Count)) %>%
  # Also convert CVP and SWP data to NA StationCode to match with salvage data
  mutate(StationCode = ifelse(Survey %in% c("CVP","SWP"), NA, StationCode))
...
# Export dataset out
write.csv(data_WY2023,file=file.path(output_root,"WY2023_catch_data.csv"))
